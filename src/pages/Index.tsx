...
// Mock Data
const mockSenders = [
  { id: "sender1", name: "Principal", number: "+5511987654321" },
  { id: "sender2", name: "Marketing", number: "+5521912345678" },
];

const mockContacts: Contact[] = [
  { id: "c1", name: "Ana Silva", number: "+5511999990001" },
  { id: "c2", name: "Bruno Costa", number: "+5521988880002" },
  { id: "c3", name: "Carla Dias", number: "+5531977770003" },
  { id: "c4", name: "Daniel Alves", number: "+5541966660004" },
];

const mockLists: ContactList[] = [
  { id: "l1", name: "Clientes VIP", memberCount: 50 },
  { id: "l2", name: "Leads (Dezembro)", memberCount: 120 },
  { id: "l3", name: "Interessados Produto X", memberCount: 75 },
];
...
const Index = () => {
  ...
  const handleListChange = (listId: string, checked: boolean) => {
    setSelectedListIds((prev) =>
      checked ? [...prev, listId] : prev.filter((id) => id !== listId)
    );
  };

  const handleSendMessage = () => {
    if (!selectedSenderId) {
      toast({ title: "Erro", description: "Por favor, selecione um remetente.", variant: "destructive" });
      return;
    }
    if (selectedContactIds.length === 0 && selectedListIds.length === 0) {
      toast({ title: "Erro", description: "Por favor, selecione ao menos um destinatário ou lista.", variant: "destructive" });
      return;
    }
    if (!message.trim()) {
